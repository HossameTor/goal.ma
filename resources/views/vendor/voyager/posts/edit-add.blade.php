        ...
        <form class="form-edit-add" role="form" action="@if($edit){{ route('voyager.posts.update', $dataTypeContent->id) }}@else{{ route('voyager.posts.store') }}@endif" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <h3 class="panel-title">Extrait</h3>
                            <div class="panel-actions">
                                <a class="panel-action voyager-angle-down" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @php
                                $dataTypeRows = $dataType->{($edit ? 'editRows' : 'addRows' )};
                                $exclude = ['title', 'body', 'excerpt', 'slug', 'status', 'category_id', 'author_id', 'featured', 'image', 'meta_description', 'meta_keywords', 'seo_title','planned'];
                            @endphp
                    ...
                    <div class="panel panel panel-bordered panel-warning">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="status" id="statut" onchange="val()">
                                    <option value="PUBLISHED"@if(isset($dataTypeContent->status) && $dataTypeContent->status == 'PUBLISHED') selected="selected"@endif>Publié</option>
                                    <option value="DRAFT"@if(isset($dataTypeContent->status) && $dataTypeContent->status == 'DRAFT') selected="selected"@endif>Brouillon</option>
                                    <option value="PENDING"@if(isset($dataTypeContent->status) && $dataTypeContent->status == 'PENDING') selected="selected"@endif>En Attente</option>
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group" id="planed">
                                <label for="name" ></label>
                                
                            </div>
                            <script>
                                function val(){
                                    if(document.getElementById("statut").value == "PENDING"){
                                        document.getElementById("planed").innerHTML = '<label for="name">Planification</label><br /><input type="datetime-local" class="form-control datepicker" name="planned" value="">';
                                    }
                                    else{
                                        document.getElementById("planed").innerHTML="";
                                    }
                                }
                            </script>
                        </div>
